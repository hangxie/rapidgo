...
func TestTreeKeyNavigationAndEditorScroll(t *testing.T) {
	...
	key(tcell.KeyF6)
	assert.Equal(t, focusOutput, state.focus, "F6 cycles tree, editor, output")
	...
	assert.Equal(t, focusTree, state.focus, "other modified F6 keys must not switch panes")
	key(tcell.KeyF1)
	key(tcell.KeyF6)
	assert.Equal(t, focusTree, state.focus, "help must consume pane switching")
	key(tcell.KeyF1)
	key(tcell.KeyF6)
	assert.Equal(t, focusEditor, state.focus)
	key(tcell.KeyPgDn)
