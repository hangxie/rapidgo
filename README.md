...
The listing is cached and dropped whenever you save, because a save can add a runnable package or change a package clause; the next run rescans. Build → Run Target always rescans, which is how to pick up a package created outside RapidGo. If a remembered target disappears from a rescan, RapidGo says so and asks again. The target is remembered for the session only and is not written to disk.
