...
## Acceptance coverage and terminal limits
...
- [x] Drop the cached runnable-package listing on save rather than holding it for the session, since a save can add a package or change a package clause and RapidGo does not watch the filesystem. Keep the chosen run target in memory only; persisting it needs a general answer to where RapidGo stores per-project state, which the MVP does not have.
- [x] Select a line in the output pane and press Enter to reach its problem, rather than stepping only between problems. Every line stays reachable, which matches the Borland message window.
