...
Command arguments are constructed directly rather than through a shell. Output that matches a known Go diagnostic is structured; all other output remains visible verbatim. A separate terminal run action suspends tcell input and rendering, runs the resolved command with the terminal streams attached, and restores RapidGo after the child exits. It requires other Go jobs to be stopped so their output cannot block while the UI is suspended. The attached child owns terminal interaction; its output is not parsed into the integrated pane.

`internal/diagnostic` parses output lines into one model of path, line, column, severity, source, package, and message. It is stateful by necessity: Go names a package once in a `# package` header above the errors in it, and marks a failing test once above the lines explaining the failure. A path is kept exactly as the tool wrote it, which may be relative to the project root, relative to a package directory for test output, or absolute; resolving it belongs to whatever navigates to it. The UI holds each problem on the output line that reported it, so trimming old output drops its diagnostics with it and the highlighted line is always the one Enter acts on. JSON test events name the package directly. For plain fallback output, a later verdict line names the package of the output above it; the view fills it in when it arrives. `go list` turns that import path into the directory a package-relative path needs. A position outside the file is clamped to it rather than refused. Go reports a column as a 1-based byte count while the editor counts grapheme clusters, so the column is converted once the file's text is available, rounding a column inside a cluster down to its start. A located line is recognized only when its path ends in `.go`, which keeps timestamps out. Indented test output also accepts bare `file.go:line:` locations and Testify's `Error Trace:` stack paths; other unmatched output stays plain. That is not enough for `go run`, whose output also carries the program's own writing and where `log.Lshortfile` prints `main.go:7: msg`. A parser told its output may be a program's records diagnostics only once a `# package` header has appeared, which means compilation failed and the program never started, so the header holds for the rest of the run. A program that prints such a header itself would be misread, which combined `go run` output cannot distinguish. A subtest's result governs only the lines indented inside it, so a parent's later output is not graded by a subtest that has ended. Test jobs decode Go JSON output, including build-output events. Go 1.27 OutputType distinguishes `t.Log` from `t.Error` and `t.Fatal`, so a log in a failing test remains informational. Go 1.26 has no OutputType; the test failure event retroactively grades that test's located output as errors, preserving the prior ambiguity without downgrading real failures. Malformed or unexpected records remain plain output. Compiler `have` and `want` continuation lines stay in the output pane and are attached to the preceding compiler diagnostic. Process cancellation must terminate child processes and prevent stale events from replacing newer results.
